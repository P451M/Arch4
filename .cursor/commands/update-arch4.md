...
8. Use staged validation before metadata generation for large DSL edits.
9. Do not invent architecture facts. If evidence is insufficient, add an open question or leave the model unchanged and explain the uncertainty.
10. Do not edit files under `.arch4/architecture/build/**`; they are derived output.
11. If an Arch4 CLI is available, run `arch4 validate`, `arch4 render`, and `arch4 index` after source edits and report the result.
12. Inspect rendered view JSON and report node, edge, and boundary counts for each view.
13. Run `git status --short` and call out unexpected changes outside `.arch4/architecture/workspace.dsl` and `.arch4/architecture/entities/*.json`.
